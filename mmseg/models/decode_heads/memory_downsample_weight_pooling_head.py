...
from mmseg.models.utils import SequenceConv
from mmseg.ops import resize
from .decode_head import BaseDecodeHead
from ..builder import HEADS


class SequenceConvSigmoid(nn.ModuleList):
    """Sequence conv and sigmoid module.
    ...
    def __init__(self, in_channels, out_channels, kernel_size, sequence_num, conv_cfg, norm_cfg, act_cfg):
        super(SequenceConvSigmoid, self).__init__()
...
class MemoryModule(nn.Module):
    ...
    def __init__(self, matmul_norm, down_sample_stride, down_sample_method, align_corners,
                 memory_keys_channels, query_key_channels, sequence_num, conv_cfg, norm_cfg, act_cfg):
        super(MemoryModule, self).__init__()
        self.matmul_norm = matmul_norm
        self.down_sample_stride = down_sample_stride
        self.align_corners = align_corners
        self.memory_keys_sequence_conv_sigmoid = SequenceConvSigmoid(memory_keys_channels, 1, 1, sequence_num,
                                                                     conv_cfg, norm_cfg, act_cfg)
        self.memory_values_sequence_conv_sigmoid = SequenceConvSigmoid(memory_keys_channels * 4, 1, 1, sequence_num,
                                                                       conv_cfg, norm_cfg, act_cfg)
        assert down_sample_method == 'average_pooling'
    ...
    def forward(self, memory_keys, memory_values, query_key, query_value):
        ...
        """
        batch_size, sequence_num, _, ori_height, ori_width = memory_keys.shape
        memory_keys_weight = self.memory_keys_sequence_conv_sigmoid(memory_keys)  # BxTx1xHxW
        # memory_keys_weight = memory_keys_weight.view(batch_size, sequence_num * ori_height, ori_width)   # BxTx1xH*W
        # memory_keys_weight = F.gumbel_softmax(memory_keys_weight, tau=1, dim=-1)
        # memory_keys_weight = memory_keys_weight.reshape(batch_size, sequence_num, 1, ori_height, ori_width)
        memory_keys = memory_keys_weight * memory_keys
        memory_values_weight = self.memory_values_sequence_conv_sigmoid(memory_values)  # BxTx1xHxW
        # memory_values_weight = memory_values_weight.view(batch_size, sequence_num * ori_height * ori_width)# BxTx1xH*W
        # memory_values_weight = F.gumbel_softmax(memory_values_weight, tau=1, dim=-1)
        # memory_values_weight = memory_values_weight.reshape(batch_size, sequence_num, 1, ori_height, ori_width)
        memory_values = memory_values_weight * memory_values
...
class MemoryDownSampWeightPoolingHead(BaseDecodeHead):
    ...
    def __init__(self, sequence_num, memory_keys_channels, query_key_channels, down_sample_stride, down_sample_method,
                 **kwargs):
        super(MemoryDownSampWeightPoolingHead, self).__init__(**kwargs)
        self.sequence_num = sequence_num
        ...
        self.memory_module = MemoryModule(False, down_sample_stride, down_sample_method, self.align_corners,
                                          memory_keys_channels, query_key_channels, sequence_num,
                                          self.conv_cfg, self.norm_cfg, self.act_cfg)
