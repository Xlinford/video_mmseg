...
class MemoryGAPModule(nn.Module):
    ...
    def forward(self, memory_keys, memory_values, query_key, query_value):
        ...
        memory_keys = [F.adaptive_avg_pool2d(batch, 1).unsqueeze(0) for batch in memory_keys]  # B, TxCx1x1
        memory_keys = torch.cat(memory_keys, dim=0).view(batch_size, sequence_num, key_channels)  # BxTxC

        query_key = F.adaptive_avg_pool2d(query_key, 1).squeeze(-1)  # BxCx1

        key_attention = torch.bmm(memory_keys, query_key)  # BxTx1
        if self.matmul_norm:
            key_attention = (key_channels ** -.5) * key_attention
        key_attention = self.act_func(key_attention).permute(0, 2, 1).contiguous()  # Bx1xT

        memory_values = memory_values.view(batch_size, sequence_num, value_channels * height * width)  # BxTxC*H*W
        memory = torch.bmm(key_attention, memory_values).squeeze(1)  # Bx1xC*H*W -> BxC*H*W
...
class MemoryGAPHead(BaseDecodeHead):
    """memory decoder with key global pooling for video semantic segmentation."""

    def __init__(self, sequence_num, memory_keys_channels, query_key_channels, act_func=None, **kwargs):
        super(MemoryGAPHead, self).__init__(**kwargs)
        self.sequence_num = sequence_num
        ...
        self.memory_module = MemoryGAPModule(act_func=act_func)
