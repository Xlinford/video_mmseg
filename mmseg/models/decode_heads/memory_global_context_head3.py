...
class MemoryModule(nn.Module):
    ...
    def forward(self, memory_keys, memory_values, query_key, query_value):
        ...
        for _batch_memory_attention, _batch_query_value in zip(g_memory_attention, query_value):
            _batch_query_value = _batch_query_value.unsqueeze(0)
            for memory_attention in _batch_memory_attention:  # TxCkxCv
                _query_value = torch.bmm(memory_attention.unsqueeze(0),
                                         _batch_query_value)  # 1xCkxH*W
                _batch_query_value = _batch_query_value + _query_value     # 1xCkxH*W
            batch_query_value.append(_batch_query_value)    # B, 1xCkxH*W

        query_value = torch.cat(batch_query_value, dim=0)  # BxCkxH*W
        query_value = query_value.view(batch_size, key_channels, height, width)  # BxT*CkxH*W
