...
class MemorySeparableHead(BaseDecodeHead):
    """Memory decoder for video semantic segmentation."""

    def __init__(self, sequence_num, key_channels, value_channels, **kwargs):
        super(MemorySeparableHead, self).__init__(**kwargs)
        self.sequence_num = sequence_num
        self.memory_key_conv = SequenceConv(self.in_channels, key_channels, 1, sequence_num,
                                            self.conv_cfg, self.norm_cfg, self.act_cfg)
        self.memory_value_conv = SequenceConv(self.in_channels, value_channels, 1, sequence_num,
                                              self.conv_cfg, self.norm_cfg, self.act_cfg)
        ...
        self.memory_module = MemoryModule(matmul_norm=True)
