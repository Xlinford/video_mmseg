...
def parse_args():
    ...
    parser.add_argument(
        '--test-fps',
        action='store_true',
        help='Whether to test model inference fps.')
    parser.add_argument(
        '--test-shape',
        type=int,
        nargs='+',
        default=[512, 512],
        help='The shape of test inference input image.')
    parser.add_argument(
        '--eval',
        type=str,
        nargs='+',
...
def main():
    ...
    assert args.out or args.eval or args.format_only or args.show \
           or args.show_dir or args.test_fps, \
        ('Please specify at least one operation (save/eval/format/show the '
         'results / save the results) with the argument "--out", "--eval"'
         ', "--format-only", "--show" or "--show-dir" pr "--test-fps"')
    ...
    if not distributed:
        ...
        if args.test_fps:
            ...
            outputs, fps, speed = single_gpu_test(model, data_loader, args.show, args.show_dir, args.test_fps)
            print('\ntest {} samples, fps: {:.2f}, speed: {:.2f} ms/f.'.format(len(outputs), fps, speed * 1000))
        else:
