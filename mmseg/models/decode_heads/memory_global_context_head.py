...
class MemoryModule(nn.Module):
    ...
    def forward(self, memory_keys, memory_values, query_key, query_value):
        ...
        # global context
        g_memory_keys = ori_memory_keys.permute(0, 2, 1, 3, 4).contiguous()  # BxCkxTxHxW
        g_memory_keys = g_memory_keys.view(batch_size, key_channels, sequence_num * height * width)  # BxCkxT*H*W
        g_memory_values = ori_memory_values.permute(0, 2, 1, 3, 4).contiguous()  # BxCvxTxHxW
        g_memory_values = g_memory_values.view(batch_size, value_channels, sequence_num * height * width)  # BxCvxT*H*W
        g_memory_values = g_memory_values.permute(0, 2, 1).contiguous()  # BxT*H*WxCv
        g_memory_attention = torch.bmm(g_memory_keys, g_memory_values)  # BxCkxCv
        g_memory_attention = F.softmax(g_memory_attention, dim=-1)

        query_value = query_value.view(batch_size, value_channels, height * width)   # BxCvxH*W
        query_value = torch.bmm(g_memory_attention, query_value)    # BxCkxH*W
        query_value = query_value.view(batch_size, key_channels, height, width)     # BxCkxHxW
        query_memory = torch.cat([query_value, memory], dim=1)
        return query_memory


@HEADS.register_module()
class MemoryGlobalContextHead(BaseDecodeHead):
    """Memory decoder for video semantic segmentation."""

    def __init__(self, sequence_num, key_channels, value_channels, **kwargs):
        super(MemoryGlobalContextHead, self).__init__(**kwargs)
        self.sequence_num = sequence_num
